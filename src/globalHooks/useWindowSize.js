...
function useWindowSize (){
    const X_BREAK_SML = 576; // 576 - 768 is sml, < is XS
    const X_BREAK_MED = 768;
    ...
    const [screenIsShort, setScreenIsShort] = useState(false);
    const [screenIsTall, setScreenIsTall] = useState(false);

    // string description of height to use for className or like situations
    const [windowHeight, setWindowHeight] = useState('');

    // preset conditions for ternary
    ...
    useEffect(() => {
        if (height < Y_BREAK_SHORT) {
            setScreenIsShort(true);
            setScreenIsTall(false);
            setWindowHeight('short');
        }
        if (height >= Y_BREAK_SHORT) {
            setScreenIsShort(false);
